...
def test_pickle_trained_model():
    ...
    # Default model case
    pm.pickle_trained_model(trained_model=MODEL, model_prefix=MODEL_PREFIX, pickle_path=tmp_dir.name)
    ...
    # H2O MOJO model case
    f = tempfile.NamedTemporaryFile(delete=False, dir=tmp_dir.name)
    pm.pickle_trained_model(trained_model=f.name, model_prefix=MODEL_PREFIX, pickle_path=tmp_dir.name, is_h2o_model=True)
    assert (Path(tmp_dir.name) / (MODEL_PREFIX + ".mojo")).exists()
    (Path(tmp_dir.name) / (MODEL_PREFIX + ".mojo")).unlink()

    # Binary string case
    binary_string = pm.pickle_trained_model(trained_model=MODEL, model_prefix=MODEL_PREFIX, is_binary_string=True)
    assert isinstance(binary_string, str)

    # MLFlow model case
    mlflow_tmp_dir = tempfile.TemporaryDirectory()
    f = tempfile.NamedTemporaryFile(delete=False, dir=mlflow_tmp_dir.name, suffix=".pickle")
