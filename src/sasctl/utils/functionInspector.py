...
def findPackageBytecodes(inst):
    ...
    if inst.opname == "LOAD_GLOBAL":
        if inst.argval in globals() and type(globals()[inst.argval]) in [
            CodeType,
            FunctionType,
        ]:
    ...
    for fn in workList:
        ...
        try:
                ...
                if var != "getFunctionPackages":
                    if inspect.ismodule(ref):
                        ...
                        if hasattr(ref, "__name__"):
                            if fn.__name__ == "getFunctionPackages":
                                logging.debug(
                                    "A- Not adding to workList %s " % fn.__name__
                                )
                            elif fn.__name__ == "findPackageBytecodes":
                                logging.debug(
                                    "A- Not adding to workList %s " % fn.__name__
            ...
            for block in codeworkList:
                for (bType, bRef) in [
                    findPackageBytecodes(inst)
                    for inst in Bytecode(block)
                    if findPackageBytecodes(inst)
                ]:
                    if bType == "modules":
                        newmods = [
                            mod.__name__ for mod in bRef if hasattr(mod, "__name__")
                        ]
                        for mo in newmods:
                        ...
                        if hasattr(bRef, "__module__"):
                            if (
                                bRef.__name__ != "getFunctionPackages"
                                or bRef.__name__ != "findPackageBytecodes"
                            ):
                                logging.debug(
                                    "3 fn=%s, bType=%s, bRef=%s" % (fn, bType, bRef)
                                )
                    ...
                    if inspect.isfunction(bRef):
                        if hasattr(bRef, "__name__"):
                            if bRef.__name__.find("getFunctionPackages") != -1:
                                logging.debug(
                                    "B- Not adding to workList %s " % bRef.__name__
                                )
                                ...
                                logging.debug(
                                    "B- Adding to workList 1: %s" % bRef.__name__
                                )
