...
The file can be associated with the URI of another identifiable object (for example, a parentUri). Every file must have
an assigned content type and name. Files can be retrieved individually by using the file's identifier or as a list of
files by using a parentUri. Each file has its content stream associated with it.  After creation, the metadata that is
...
list_files, get_file, update_file, delete_file = _build_crud_funcs(_SERVICE_ROOT + '/files', 'file')
...
def create_file(file, folder=None, filename=None, expiration=None):
    ...
    return post(_SERVICE_ROOT + '/files#multipartUpload', files={filename: file}, params=params)
...
def get_file_content(file):
    ...
    return request_link(file, 'content')
