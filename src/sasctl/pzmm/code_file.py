...
class CodeFile(Service):
    ...
    @classmethod
    def _extract_docstring_variables(
        cls, code: str, docstring_prefix: str
    ) -> List[str]:
        """
        ...
        if not matching_lines:
            raise ValueError(
                f"Code must contain '{cls.OUTPUT_DOCSTRING_PREFIX}' docstring. "
            )
        ...
        return [
            var.strip("'").strip() for var in variables_str.split(",") if var.strip()
        ]
    ...
    def _validate_return_consistency(cls, tree: ast.AST) -> int:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Return):
                ...
                if (
                    return_values_count is not None
                    and return_values_count != current_count
                ):
                    raise ValueError(
    ...
    def _validate_output_docstring(cls, code: str, tree: ast.AST):
        ...
        output_variables = cls._extract_docstring_variables(
            code, cls.OUTPUT_DOCSTRING_PREFIX
        )
    ...
    def _validate_dependency_docstring(cls, code: str):
        ...
        dependency_differences = set(detected_dependencies).difference(
            normalized_docstring_deps
        )
        ...
        if dependency_differences:
            ...
            for dep in sorted(dependency_differences):
                ...
                if install_name != dep:
                    missing_deps_with_install_names.append(
                        f"'{install_name}' (imported as '{dep}')"
                    )
                else:
    ...
    def validate_id_code(
        cls, code: Union[str, Path], validate_code: bool = True
    ) -> str:
        """
        ...
        if not has_execute_function:
            raise ValueError(
                f"Code must contain an '{cls.EXECUTE_FUNCTION_NAME}' function"
            )
