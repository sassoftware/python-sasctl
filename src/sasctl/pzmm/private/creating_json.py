...
for i, label in enumerate(reversed(prob_values)):
    json_dict[1]['parameterMap'][f'predict_proba{i}']['label'] = f'Average Predicted: {actual_valule}={i}'
    json_dict[1]['parameterMap'][f'predict_proba{i}']['parameter'] = label
    json_dict[1]['parameterMap'][f'predict_proba{i}']['values'] = [label]

    json_dict[1]['parameterMap'] = {label if k == f'predict_proba{i}' else k: v for k, v in  json_dict[1]['parameterMap'].items()}
