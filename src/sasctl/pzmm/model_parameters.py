    ...
    def generate_hyperparameters(
        ...
        def tf_params():
            ...
            if not model_metadata:
                raise RuntimeError("Tensorflow is required to generate keras hyperparameters.")
            hyperparameters = model_metadata(model)
            model_json = {"hyperparamters": hyperparameters}
            with open(
                Path(pickle_path) / f"{model_prefix}Hyperparameters.json", "w"
            ) as f:
                f.write(json.dumps(model_json, indent=4))
        def xg_params():
            ...
            """
            if not xgboost:
                raise RuntimeError("XGBoost is required to generate xgboost hyperparameters.")
