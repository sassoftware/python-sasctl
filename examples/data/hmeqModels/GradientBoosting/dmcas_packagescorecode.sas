...
method score(double LOAN, double MORTDUE, double VALUE, double YOJ, double DEROG, double DELINQ, double CLAGE, double NINQ, double CLNO, double DEBTINC, in_out double resultCode, in_out double EM_EVENTPROBABILITY, in_out varchar(100) EM_CLASSIFICATION);
   ...
   if null(pm) then do;
      pm = _new_ pymas();
      resultCode = pm.useModule('model_exec_b30dbd5b-aac8-48d8-89a6-79dd652541cc', 1);
      if resultCode then do;
         ...
         resultCode = pm.appendSrcLine('');
         resultCode = pm.appendSrcLine('with open("/models/resources/viya/45b956e1-1aed-4c7a-a8dd-82e8676c2a23/GradientBoosting.pickle", "rb") as _pFile:');
         ...
         revision = pm.publish(pm.getSource(), 'model_exec_b30dbd5b-aac8-48d8-89a6-79dd652541cc');
